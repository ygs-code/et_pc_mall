  ...
  <div>
    <div class="order-detail wrapper_1200">
      <div class="header">
        ...
        <nuxt-link :to="{ path: '/users/order_list', query: { menuCur: 111 } }"
          >Individual Center</nuxt-link
        >
      ...
      <div class="section process mb15">
        <div class="section-hd presell-hd">
          <div v-show="orderStatus === 0" class="state">
            {{ $t(`page.user.orderStatus[0].name`) }}
          </div>
        ...
        <div class="section-bd">
          <ul>
            <li class="acea-row row-middle">
              ...
              <div>
                {{ isGoodsReturn ? orderInfo.merOrderNo : orderInfo.orderNo }}
              </div>
            </li>
            <li class="acea-row row-middle">
            ...
            <li v-if="!isGoodsReturn" class="acea-row row-middle">
              ...
              <div class="money">
                {{ GLOBAL.shopPayCurrency }}{{ orderInfo.payPrice }}
              </div>
            </li>
      ...
      <div v-if="Number(orderInfo.status) > 0" class="section mb15">
        <el-collapse>
          <el-collapse-item
            :title="
              $t(`page.users.logistics.expName`) +
              '：' +
              expressInfo.expName +
              '     ' +
              $t(`page.users.logistics.expNo`) +
              '：' +
              expressInfo.expNo
            "
            name="1"
            :disabled="expressList.length === 0"
          >
            <div class="logisticsCon">
              ...
              >
                <div
                  class="iconfont icon-a-bianzu2 circular"
                  :class="index === 0 ? 'on' : ''"
                ></div>
                <div class="text" :class="index === 0 ? 'on-font on' : ''">
                  <div>{{ item.AcceptStation }}</div>
                  <div class="data" :class="index === 0 ? 'on-font on' : ''">
                    {{ item.AcceptTime }}
                  </div>
                </div>
              </div>
        ...
        <div class="section-bd order_price">
          <ul class="cor666 p30">
            ...
            >
              ...
              <div>
                <span
                  >{{ GLOBAL.shopPayCurrency }}{{ orderInfo.payPostage }}</span
                >
              </div>
            </li>
          </ul>
          <ul class="total cor666">
            <li class="acea-row row-middle row-between">
              <div>
                {{ orderInfo.totalNum }}
                {{ $t(`page.users.orderList.item`) }}，{{
                  $t(`page.users.orderList.totalPay`)
                }}：<span class="money"
                  >{{ GLOBAL.shopPayCurrency
                  }}<b>{{ orderInfo.payPrice }}</b></span
                >
              </div>
              <div class="footerState acea-row row-middle">
                <div
                  class="orderBnt"
                  v-if="orderInfo.paid === true && orderInfo.refundStatus === 0"
                  @click="refund(orderInfo.orderNo)"
                >
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button size="small" type="primary" @click="getPaymentStatus">{{
          $t(`page.users.orderConfirm.paymentResults`)
        }}</el-button>
      </span>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    getOrderMasterInfo() {
      this.$axios
        .get(`/api/front/order/master/detail/${this.orderNo}`)
        .then((res) => {
    ...
    getOrderInfo() {
      this.$axios
        .get(`/api/front/order/detail/${this.orderNo}`)
        .then((res) => {
          this.orderInfo = res.data;
    ...
    getExpress() {
      this.$axios
        .get(`/api/front/order/express/${this.orderNo}`)
        .then((res) => {
    ...
    goOrderConfirm: Debounce(function () {
      ...
      this.$axios
        .post("/api/front/cart/save", storeCartListRequest)
        .then((res) => {
