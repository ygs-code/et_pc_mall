                ...
                <div class="goods-main">
                    <div class="acea-row row-top" style="position: relative;">
                        <div class="carousel">
                            ...
                            <client-only>
                                <div v-swiper:carousel="swiperOption">
                                    <div class="swiper-wrapper">
                                        <div v-for="(item, index) in JSON.parse(productInfo.sliderImage)" :key="index"
                                            :class="{ on: slideIndex === index }" class="swiper-slide"
                            ...
                            ]"></span>{{ userCollect ? $t(`page.store.followed`) : $t(`page.store.follow`) }}
                                </div>
                            </div>
                        ...
                        <div class="text-wrapper">
                            ...
                            <div class="attribute">
                                <div v-for="(item, index) in productAttr" :key="index" class="acea-row size-wrapper">
                                    <div class="label">{{ item.attrName }}</div>
                                    <div class="acea-row list">
                                        <label v-for="(itm, idx) in item.attrValues.split(',')" :key="idx" class="item">
                                            <input v-model="attrSelected[index]" type="radio" :name="index" :value="itm"
                                                :checked="!idx" hidden />
                            ...
                            <div class="number-wrapper acea-row">
                                ...
                                <div class="counter-wrap acea-row">
                                    <div class="counter">
                                        <button class="iconfont icon-shangpinshuliang-jian"
                                            :disabled="count === 1 || !stock" @click="minus">—</button>
                                        <input v-model="count" @input="inputNum" />
                                        <button style="font-size: 20px;" class="iconfont icon-shangpinshuliang-jia"
                                            :disabled="count === stock || !stock" @click="plus">+</button>
                                    </div>
                                    <span>{{ $t(`page.goodsDetail.inventory`) }}：{{ stock || 0 }}{{ productInfo.unitName
                                ||
                                '' }}</span>
                                </div>
                            </div>
                            <div class="button-wrapper" v-if="stock">
                                <button class="btn cart" :disabled="!stock" @click="buy(1, $event)">
                            ...
                            <div v-if="guaranteeList.length" class="number-wrapper acea-row guaranteeList">
                                <div class="label" @mouseover="guaranteeTitle = true"
                                    @mouseleave="guaranteeTitle = false">{{ $t(`page.goodsDetail.assure`) }} <span
                                        class="iconfont icon-duoshanghupc-shuomingdanchuang"></span></div>
                                <div class="guaranee_tel"
                                    :style="{ display: (guaranteeTitle || guaranteeInfo) ? 'block' : 'none' }"
                            ...
                            <div class="detail-bd">
                                    ...
                                    <div class="comment-hd">
                                        <div class="acea-row row-between-wrapper">
                                            <div class="acea-row row-middle score">
                                                ...
                                                <div class="cont">
                                                    <el-rate v-model="replyInfo.replyStar" disabled
                                            ...
                                            <div v-for="item in replyList" :key="item.reply_id" class="item">
                                                ...
                                                <div class="item-bd">
                                                    ...
                                                    <div class="image-wrapper"
                                                        ...
                                                        <div v-for="(itm, idx) in item.pics" :key="idx" class="image"
                                                            @click="isDialog = true">
                                                            <el-image style="width: 54px; height: 54px" :src="itm"
                                                                :preview-src-list="item.pics"></el-image>
                                                        </div>
                                                    </div>
                                                    <div v-if="item.merchantReplyContent" class="reply">
                                                        <div class="item">
                                                            <span>{{ $t(`page.users.goodsCommentList.seller`)
                                                                }}：</span>{{
                                item.merchantReplyContent }}
                                    ...
                                    <div v-if="replyList.length" class="acea-row row-right">
                                        <el-pagination layout="prev, pager, next" :page-size="reply.limit"
                                            :total="replyCount" @current-change="callPaginate"
                                            @prev-click="callPaginate" @next-click="callPaginate"></el-pagination>
                                    </div>
                                </div>
                                <div class="store-recommend" v-if="goodsList && goodsList.length">
                                    ...
                                    <div class="list acea-row row-around">
                                        <nuxt-link v-for="(item, index) in goodsList" :key="index"
                                            :to="`/goods_detail/${item.id}`" class="r-l-item">
                                            <div class="image">
                                                <img :src="item.image" />
                                            </div>
                                            <div class="text">
                                                ...
                                                <div class="acea-row row-between-wrapper">
                                                    ...
                                                    <div class="sales">{{ (Math.floor(item.sales) +
                                                        Math.floor(item.ficti))
                                                        || 0 }} {{ $t(`message.tips.sold`) }}</div>
                                                </div>
                                            </div>
                                        </nuxt-link>
                                    </div>
